...
bot.start((ctx) => {
  ctx.reply(`Olá, ${ctx.from.first_name}, bem-vindo ao bot da liturgia das horas. v1.0.10`)
  contextChatId = ctx.chat.id
}) 
...
new CronJob('0 4,9,12,15,18 * * 0-6', function() {
  ...
  if(contextChatId) {
    bot.telegram.sendMessage(contextChatId,  `${currentHour}h - rezar ${currentPrayer}`)
  }
}, null, true, tz);
